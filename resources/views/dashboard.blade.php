...
<div class="container-fluid mt-4">
    ...
    {{-- Alert Selamat Datang Berdasarkan Role --}}
    <div class="alert alert-{{ Auth::user()->role === 'owner' ? 'success' : 'primary' }} shadow-sm border-0 d-flex align-items-center alert-dismissible fade show">
        <div class="bg-white bg-opacity-25 rounded-circle p-2 me-3">
            <i class="fa-solid fa-user"></i>
        </div>
        <div class="flex-grow-1">
            Selamat datang, <strong>{{ Auth::user()->name }}</strong>!
            Anda masuk sebagai <strong>{{ Auth::user()->role === 'owner' ? 'Owner' : 'Kasir' }}</strong>.
        </div>
    ...
    <div class="alert alert-info shadow-sm border-0 d-flex align-items-center justify-content-between">
        <div class="d-flex align-items-center flex-grow-1">
            ...
            <div>
                ...
                <small>
                    Dibuka oleh <strong>{{ $activeShift->user->name ?? 'Kasir' }}</strong>
                    pada <strong>{{ $activeShift->date->format('d/m/Y H:i') }}</strong>
                    dengan kas awal <strong>Rp {{ number_format($activeShift->opening_amount, 0, ',', '.') }}</strong>
                </small>
            </div>
        </div>
        <div>
            <a href="{{ route('kas.closing.form') }}" class="btn btn-sm btn-danger">
    ...
    <div class="alert alert-warning shadow-sm border-0 d-flex align-items-center">
        ...
        <div>
            ...
            <a href="{{ route('kas.opening.form') }}" class="fw-bold text-decoration-underline">buka kasir baru</a> untuk memulai shift.
        </div>
    </div>
    @endif
            ...
            <form method="GET" class="row g-3">
                <div class="col-lg-4 col-md-5 col-sm-6">
                    ...
                    <input type="date" id="tanggal_awal" name="tanggal_awal" class="form-control"
                        value="{{ request('tanggal_awal', $tanggalAwal->format('Y-m-d')) }}">
                </div>
                <div class="col-lg-4 col-md-5 col-sm-6">
                    <label for="tanggal_akhir" class="form-label fw-semibold small mb-2">Sampai Tanggal</label>
                    <input type="date" id="tanggal_akhir" name="tanggal_akhir" class="form-control"
                        value="{{ request('tanggal_akhir', $tanggalAkhir->format('Y-m-d')) }}">
                </div>
                <div class="col-lg-4 col-md-2 col-sm-12">
                    ...
                    <div class="d-flex gap-2">
                        ...
                        <a href="{{ route('dashboard.index') }}" class="btn btn-outline-secondary flex-fill">
                ...
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-start mb-3">
                        <div class="flex-grow-1">
                            ...
                            <h4 class="fw-bold mb-2">Rp {{ number_format($item['value'], 0, ',', '.') }}</h4>
                            @if($item['status'] === 'up')
                                <small class="text-success">
                                    <i class="fa-solid fa-arrow-up"></i> {{ round($item['percent'],1) }}% dari periode sebelumnya
                                </small>
                            @elseif($item['status'] === 'down')
                                <small class="text-danger">
                                    <i class="fa-solid fa-arrow-down"></i> {{ abs(round($item['percent'],1)) }}% dari periode sebelumnya
                                </small>
                            @else
                        ...
                        <div class="bg-{{ $item['color'] }} bg-opacity-10 rounded-3 p-3">
                            <i class="fa-solid fa-{{ $item['icon'] }} text-{{ $item['color'] }} fs-4"></i>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <h4 class="fw-bold mb-0">{{ number_format($s['value'], 0, ',', '.') }}{{ $s['suffix'] }}</h4>
                        </div>
                        <div class="bg-{{ $s['color'] }} bg-opacity-10 p-3 rounded-3">
                            <i class="fa-solid fa-{{ $s['icon'] }} text-{{ $s['color'] }} fs-4"></i>
                        </div>
                    </div>
                </div>
            </div>
    ...
    {{-- Metode Pembayaran dan Produk Terlaris --}}
    <div class="row g-4 mb-4">
        {{-- Metode Pembayaran --}}
        <div class="col-lg-6">
            <div class="card shadow-sm border-0 h-100">
                            ...
                            <div class="p-3 bg-primary bg-opacity-10 rounded-3">
                                <div class="d-flex align-items-center justify-content-between">
                                    <div>
                                        ...
                                        <h5 class="fw-bold mb-0 text-primary">Rp {{ number_format($pendapatanTunai ?? 0, 0, ',', '.') }}</h5>
                                    </div>
                            ...
                            <div class="p-3 bg-info bg-opacity-10 rounded-3">
                                <div class="d-flex align-items-center justify-content-between">
                                    <div>
                                        ...
                                        <h5 class="fw-bold mb-0 text-info">Rp {{ number_format($pendapatanNonTunai ?? 0, 0, ',', '.') }}</h5>
                                    </div>
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <span class="badge bg-success bg-opacity-10 text-success">
                                            {{ number_format($item->total_qty ?? 0, 0, ',', '.') }} item
                                        </span>
                                    </td>
                                </tr>
                                @empty
...
<script>
    ...
    new Chart(jenisCtx, {
        ...
        data: {
            ...
            datasets: [{
                data: [{{ $pendapatanTunai ?? 0 }}, {{ $pendapatanNonTunai ?? 0 }}],
