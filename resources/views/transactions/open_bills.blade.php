...
<div class="d-flex justify-content-between align-items-center mb-3">
    ...
    <a href="{{ route('transaksi.index') }}" class="btn btn-outline-primary"><i class="fa-solid fa-cash-register me-1"></i> Kembali ke Transaksi</a>
    </div>
          ...
          <tbody id="openBillTableBody">
            @foreach($bills as $bill)
              <tr data-id="{{ $bill->id }}">
                ...
                <td>
                  <span class="badge bg-info">{{ $bill->user->name ?? '-' }}</span>
                </td>
                <td>{{ $bill->customer_name ?: '-' }}</td>
                <td>{{ $bill->order_type === 'take_away' ? 'Take Away' : 'Dine In' }}</td>
                <td>Rp {{ number_format($bill->total,0,',','.') }}</td>
                <td>{{ $bill->created_at ? $bill->created_at->format('d M Y H:i') : '-' }}</td>
                <td>
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  body.querySelectorAll('.btn-load').forEach(btn => {
    btn.onclick = function(){
      ...
      fetch("{{ route('transaksi.openBill.load', ['id' => '__ID__']) }}".replace('__ID__', id), {
      ...
      .then(json => {
        if(json.success){
          Swal.fire({ icon: 'success', title: 'Dimuat', timer: 800, showConfirmButton: false })
            .then(() => window.location.href = "{{ route('transaksi.index') }}");
        } else {
  ...
  body.querySelectorAll('.btn-delete').forEach(btn => {
    btn.onclick = function(){
      ...
      Swal.fire({
        ...
        fetch("{{ route('transaksi.openBill.destroy', ['id' => '__ID__']) }}".replace('__ID__', id), {
          method: 'DELETE',
