    ...
    <div class="row g-3 g-lg-4">
        ...
        @foreach($products as $product)
            ...
            <div class="col-6 col-sm-4 col-md-4 col-lg-3 menu-item"
                ...
                data-has-stock="{{ $product->has_stock ? 1 : 0 }}"
                data-stock="{{ (int)($product->stock ?? 0) }}">
            <div class="card menu-card h-100 shadow-sm border-0 rounded-3 overflow-hidden position-relative {{ $isOut ? 'opacity-75' : '' }}" style="transition: transform 0.2s ease, box-shadow 0.2s ease; cursor: pointer;">
                @if($product->image)
                ...
                <span class="position-absolute top-0 start-0 m-2 badge bg-warning text-dark rounded-pill" style="font-size: 0.75rem;">
                    <i class="fa-solid fa-exclamation-triangle me-1"></i>Stok {{ (int)($product->stock ?? 0) }}
                </span>
                @endif
                <div class="card-body text-center py-2">
                    <h6 class="card-title fw-bold mb-1 text-truncate" style="font-size: 0.85rem; color: #212529;">{{ $product->name }}</h6>
                    <p class="text-success fw-semibold mb-0" style="font-size: 0.9rem;">Rp {{ number_format($product->price,0) }}</p>
                </div>
            </div>
        </div>
        @endforeach
                ...
                <div class="card-body d-flex flex-column p-3" style="flex: 1; overflow-y: auto; gap: 0.5rem;">
                    ...
                    <div class="mb-2">
                        ...
                        <input type="text"
                               ...
                               value="{{ $cartOrderInfo['customer_name'] ?? '' }}"
                    ...
                    <div class="mb-2">
                        ...
                        <select id="orderType" class="form-select rounded-2 border-1" style="font-size: 0.9rem; padding: 0.5rem 0.6rem;">
                            <option value="dine_in" {{ ($cartOrderInfo['order_type'] ?? '') === 'dine_in' ? 'selected' : '' }}>Dine In</option>
                            <option value="take_away" {{ ($cartOrderInfo['order_type'] ?? '') === 'take_away' ? 'selected' : '' }}>Take Away</option>
                        </select>
                    </div>
                    ...
                    <div class="mb-2">
                        ...
                        <div class="input-group rounded-2" style="overflow: hidden;">
                            ...
                            <select id="discountType" class="form-select border-1" style="max-width: 75px; font-size: 0.9rem; padding: 0.5rem;">
                                <option value="percent" {{ ($cartDiscount['type'] ?? '') === 'percent' ? 'selected' : '' }}>%</option>
                                <option value="nominal" {{ ($cartDiscount['type'] ?? '') === 'nominal' ? 'selected' : '' }}>Rp</option>
                            </select>
                    ...
                    {{-- Daftar Item (Flex Grow dengan Min Height) --}}
...
{{-- Modal Open Bill (Updated Modern UI) --}}
...
document.addEventListener('DOMContentLoaded', function() {
    // Init from server
    let cart = @json($cart ?? []);
    const serverDiscount = @json($cartDiscount ?? ['value'=>0,'type'=>'percent']);
    const serverOrderInfo = @json($cartOrderInfo ?? ['customer_name'=>'','order_type'=>'dine_in']);
    const openBillsFromServer = @json($openBills ?? []);
    ...
    // URLs and CSRF
    const addUrlBase = "{{ url('transaksi/add') }}";
    const updateCartUrl = "{{ route('transaksi.updateCart') }}";
    const saveOpenBillUrl = "{{ route('transaksi.openBill.store')}}";
    const openBillListUrl = "{{ route('transaksi.openBill.list') }}";
    const openBillLoadUrlBase = "{{ url('transaksi/open-bill/load') }}";
    const openBillDeleteUrlBase = "{{ url('transaksi/open-bill/delete') }}";
    const checkoutUrl = "{{ route('transaksi.checkout') }}";
